...
function get_bt_search_by_title($query, $curr_page)
{
	...
	if ( $curr_page > 1 )
	{
		$spage = $curr_page - 1;
		if ( $numpages >= 4 )
			$first = ' <a href="?btp=1&type=bt&title=' . $query . '">&laquo;首頁</a> ';
		else
			$first = '';
	} else {
	...
	if ( $curr_page < $numpages )
	{
		$spage = $curr_page + 1;
		if ( $numpages >= 4 )
			$last = ' <a href="?btp=' . $numpages . '&type=bt&title=' . $query . '">末頁&raquo;</a> ';
		else
			$last = '';
	} else {
...
function get_bt_search_by_owner($query, $curr_page)
{
	...
	if ( $curr_page > 1 )
	{
		$spage = $curr_page - 1;
		if ( $numpages >= 4 )
			$first = ' <a href="?btp=1&type=bt&owner=' . $query . '">&laquo;首頁</a> ';
		else
			$first = '';
	} else {
		$prev = '&nbsp;'; // We're on page one, no need to print previous link
	...
	if ( $curr_page < $numpages )
	{
		$spage = $curr_page + 1;
		if ( $numpages >= 4 )
			$last = ' <a href="?btp=' . $numpages . '&type=bt&owner=' . $query . '">末頁&raquo;</a> ';
		else
			$last = '';
	} else {
		$next = '&nbsp;'; // We're on the last page
